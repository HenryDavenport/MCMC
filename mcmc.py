...
class MCMC:
    ...
    Background_Input_Data = {"background": 70000}
    ...
    def __gen_input_dict(self, input_data, template):
        ...
        if self.Fit_Background:
            total_dict.append(self.Background_Input_Data)
    ...
    e.g. background, asymmetry etc."""
        ...
        for [l, n] in list1:
            ...
            if self.Different_Widths:
                pars.extend(["linewidth"])
            if self.Different_Asymmetries:
                pars.extend(["asymmetry"])
            pars.append(peak)
    ...
    def __gen_par_values(self, dict_list, listPar, list_Global):
        ...
        # TODO FIX THIS TO ADD LOOK FOR PEAK IDENTIFIER STUFF
    ...
    def run(self, all_data, initial_guess_dicts, list_ln_numbers, prior_dict, folder_name, lock, return_dict, pars_constant, pow_Win):
        """runs fitting on pair of peaks in input"""
        # correct powers if scaling/normalisation differences
        initial_pars_constant = pars_constant
        freq_peaks1, power_peaks1 = all_data
        Final_Values_Dict_non_log, Final_Fit_Values_log, log_samples, Par_List, Global_List, constants_dict = self.__run_sampler(
            freq_peaks1, power_peaks1, folder_name, initial_guess_dicts, list_ln_numbers, lock, pars_constant, prior_dict, pow_Win)
        print(Final_Values_Dict_non_log)
        ...
        Final_Values_Dict_log = self.__gen_dict_from_pars(Final_Fit_Values_log, constants_dict, Par_List_Logs, Global_List_Logs,
                                                   list_ln_numbers)
        ...
        Final_Errors_Dict = self.__gen_dict_from_pars(self.__get_errors(log_samples, Final_Fit_Values_log), constants_dict,
                                               Par_List_Logs_errors,
        ...
        output_template = ["l", "n", "freq", "freq error", "ln(power)", "ln(power) error", "splitting", "splitting error",
                           "scale",
    ...
    def __run_sampler(self, freq_1, power_1, folder_name, initial_guess_pars, list_ln_numbers, lock, pars_constant, prior_dict, pow_Win):
        ...
        # plt.plot(freq_1, power_1, label="Simulated data")
        # plt.plot(freq_1, all_peaks_model(freq_1, Final_Values_Dict_non_log), label="MCMC Fit")
        # plt.show()
    ...
    def fit_all_peaks(self, power, freq, foldername, freq_range=(2000, 3500), pars_constant=parameters_kept_constant,
        ...
        # find all peaks in this frequency range:
        ...
        peaks_fitted_together = [list_of_peaks_in_range[i:i + modes_per_fit] for i in range(0, len(list_of_peaks_in_range), 1) if i + modes_per_fit <= len(list_of_peaks_in_range)]
